...
class FinaliseEmitsTest(TestCase):
    ...
    @mock.patch("cortex_job.cortex_utils.cortex_and_job_management.CortexJobManager.generate_description")
    @mock.patch(
        "cortex_job.cortex_utils.cortex_and_job_management.CortexJobManager.get_results",
        return_value={"total": 1},
    )
    def test_manage_jobs_emits_when_done(self, _r, _d, _g):
        from cortex_job.cortex_utils.cortex_and_job_management import CortexJobManager
        user = make_user()
        case = Case.objects.create(reporter=user, status="Done", description="t")
        with mock.patch(
            "cortex_job.cortex_utils.cortex_and_job_management.emit_connector_event"
        ) as emit_mock:
    ...
    def test_manage_jobs_no_emit_when_not_done(self, _r, _d, _g):
        from cortex_job.cortex_utils.cortex_and_job_management import CortexJobManager
        user = make_user()
        case = Case.objects.create(reporter=user, status="To Do", description="t")
        with mock.patch(
            "cortex_job.cortex_utils.cortex_and_job_management.emit_connector_event"
        ) as emit_mock:
            CortexJobManager().manage_jobs(case)
