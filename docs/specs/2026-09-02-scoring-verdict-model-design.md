...
| Verdict | Rule |
|---|---|
| **Dangerous** | a Tier-1 source says `malicious` with confidence ≥ `HIGH_CONFIDENCE` (default 70), or with no numeric confidence (tier alone carries it) · **or** ≥ 2 Tier-2 sources say `malicious` · **or** trust-weighted `malicious` share ≥ `DANGEROUS_SHARE` (default 0.5) **from a set that includes at least one Tier-1/2 `malicious` voter** (a Tier-3-only consensus, however large, caps at Suspicious) |
